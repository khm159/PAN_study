...
class BNInception(nn.Module):
    ...
    def features(self, x):
        ...
        # stage2
        print('pool1_3x3_s2_out',pool1_3x3_s2_out.shape)
        print('input',self._temporal_forward_wrap(self._block_2, 1)(pool1_3x3_s2_out).shape)
    ...
    def _temporal_forward_wrap(self, layer_func, index):
        if hasattr(self, 'is_temporal_shift') and self.is_temporal_shift[index] == '1':  # run temporal shuffling
            ...
            def wrapped_func(x, is_residual, n_segment, fold_div):
                print('wrapped_func','x',x.shape,'n_segment',n_segment)
                if is_residual:
                    print('is_residual')
                    x_shift = TemporalShift.shift(x, n_segment, fold_div=fold_div)
                    print('x.shape : ',x.shape)
                    print('x_shift :', x_shift.shape)
                    print('layer_func : ',layer_func(x_shift).shape)
                    print('x+layer_func(x_shift) :', x+layer_func(x_shift))
                    
                    return F.relu(x + layer_func(x_shift))
                else:
                    print('is_not_residual')
